...
def rollback_cmd(ctx: click.Context) -> None:
    ...
    try:
        ...
        if deleted_count > 0:
            ...
            click.secho(f"\n✅ Đã rollback thành công! Đã xóa sạch {deleted_count} leads từ đợt cào gần nhất ra khỏi Database.{deleted_file_msg}", fg="green", bold=True)
        else:
            click.secho("\n⚠️ Không tìm thấy đợt cào nào để rollback, hoặc Database đang trống.", fg="yellow")
