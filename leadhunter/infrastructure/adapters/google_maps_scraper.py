...
class GoogleMapsScraper:
    ...
    def __init__(self, http_timeout: float = 15.0, proxy: dict[str, str] | None = None, api_key: str | None = None) -> None:
    ...
    def scrape_fast(self, keyword: str, min_clean_target: int = 80, status_callback=None, is_duplicate_fn=None, worker_id: int = 1, stop_event=None) -> list[dict[str, str]]:
        ...
        def _extract_address(snippet: str) -> str:
            ...
            for a in addr_m:
                a_clean = a.strip()
                if "http" not in a_clean and "google.com" not in a_clean and not a_clean.startswith("0a"):
                    return a_clean
        ...
        def handle_response(response: Response) -> None:
            ...
            try:
                ...
                raw_matches = re.findall(r'(?:\+84|84|0)\s*(?:3|5|7|8|9)\d[\d\s.\-]{7,11}\d', text)
                for raw in raw_matches:
                    cleaned = raw.strip().replace(" ", "").replace("-", "").replace(".", "").replace("+84", "0")
                    if cleaned.startswith("84") and len(cleaned) >= 10:
                        cleaned = "0" + cleaned[2:]
                    if len(cleaned) != 10 or not cleaned.startswith(("03", "05", "07", "08", "09")):
                        continue
        ...
        def _is_clean_phone(p_str: str) -> tuple[bool, str]:
            cleaned = p_str.strip().replace(" ", "").replace("-", "").replace(".", "").replace("+84", "0")
            if cleaned.startswith("84") and len(cleaned) >= 10:
                cleaned = "0" + cleaned[2:]
        ...
        def _is_professional_website(url: str) -> bool:
            ...
            allowed_links = [
                ...
                "google.com", "facebook.com", "zalo.me", "tiktok.com", 
                "youtube.com", "shopee.vn", "lazada.vn", "chotot.com", 
                "instagram.com", "gg.gg", "blogspot.com", "kiotvietweb.vn",
                # Trang danh bạ doanh nghiệp, mã số thuế (Thêm mới)
                "hosocongty.vn", "hosocongty.com", "masothue.com", "masothue.vn",
                "trangvangvietnam.com", "bizz.vn", "hosodoanhnghiep.vn", 
                "fact-link.com", "thongtindoanhnghiep.co", "bizs.vn",
        ...
        for district in districts:
            ...
            for label, proxy_cfg in attempts:
                ...
                if leads:
                    ...
                    logger.info(f"Collected total {len(all_leads)} leads ({phone_count} with phone)")
                    break
                logger.warning(f"Google Maps [{label}] returned 0 results, trying next…")
    ...
    def _try_scrape(self, url: str, proxy_cfg: dict | None) -> list[dict[str, str]]:
        ...
        try:
            with sync_playwright() as p:
                ...
                listing_links = page.query_selector_all("a.hfpxzc") or page.query_selector_all("a.HFpxzc")
                items = []
                ...
                leads = []
                logger.info(f"Found {len(items)} listing links, ultra-fast extracting details...")
                ...
                for i, item in enumerate(items[:150]):
                    try:
                        ...
                        if not phone_btn:
                            phone_btn = page.query_selector("button[aria-label*='Phone']")
                        if not phone_btn:
                            phone_elements = page.query_selector_all("button[data-item-id*='phone']")
                            if phone_elements:
                                phone_btn = phone_elements[0]

                        if phone_btn:
                            phone_text = phone_btn.get_attribute("aria-label") or phone_btn.inner_text()
                            phone_match = re.search(r"((?:\+84|0)[\d\s\-\.]{8,14})", phone_text)
                            if phone_match:
                        ...
                        if addr_btn:
                            ...
                            addr_text = re.sub(r"^(Address|Địa chỉ):\s*", "", addr_text, flags=re.IGNORECASE)
                            addr_text = re.sub(r"[\ue000-\uefff]", "", addr_text).strip()
                            if addr_text:
                                if not any(x in addr_text.lower() for x in ["hồ chí minh", "hcm"]):
                                    addr_text = f"{addr_text}, TP. Hồ Chí Minh"
                                address = addr_text
                        ...
                        if phone:
                            logger.info(f"  [{i+1}/{len(items)}] {name} | phone={phone}")
        ...
        for link in links:
            try:
                ...
                if parent:
                    ...
                    full_text = " | ".join(d.get_text(" ", strip=True) for d in detail_divs)
                    ...
                    for d in detail_divs:
                        ...
                        if any(x in text.lower() for x in ["đ.", "đường", "phường", "quận", "tân", "bình", "gò vấp"]):
                            # Clean up: remove category prefix like "Spa · "
                            parts = text.split("·")
                            for part in parts:
                                part = part.strip()
                                if any(c.isdigit() for c in part) or any(
                                    x in part.lower() for x in ["đ.", "đường", "phường", "quận"]
                                ):
                                    street = part.strip()
